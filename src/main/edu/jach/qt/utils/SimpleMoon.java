...
public class SimpleMoon implements TimerListener
{
	...
	/**
	 * Calculate the right ascension and declination of the moon at a apecified date.. 
	 * Uses the formaula in Astronomical Almanacs for calculating the approximate position.
	 * 
	 * @param isoDateTime
	 *            <code>String</code> giving the current time in ISO format (yyyy-mm-dd'T'HH:MM:SS)
	 */
	 ...
	 * 
	 * @param _cal
	 *            <code>Calendar</code> giving the current time.
	 */
	private void getCurrentPosition( Calendar _cal )
	{
		...
		/*
		 * Use formula in Ast. Alm. to compute ecliptic longitude (lamda), ecliptic latitude (beta) and horizontal paralax (pi)
		 */
		double lambda = 218.32 + 481267.883 * jCent + 6.29 * Math.sin( ( 134.9 + 477198.85 * jCent ) / DINR ) - 1.27 * Math.sin( ( 259.2 - 413335.38 * jCent ) / DINR ) + 0.66 * Math.sin( ( 235.7 + 890534.23 * jCent ) / DINR ) + 0.21 * Math.sin( ( 269.9 + 954397.70 * jCent ) / DINR ) - 0.19 * Math.sin( ( 357.5 + 35999.05 * jCent ) / DINR ) - 0.11 * Math.sin( ( 186.6 + 966404.05 * jCent ) / DINR );

		double beta = 5.13 * Math.sin( ( 93.3 + 483202.03 * jCent ) / DINR ) + 0.28 * Math.sin( ( 228.2 + 960400.87 * jCent ) / DINR ) - 0.28 * Math.sin( ( 318.3 + 6003.18 * jCent ) / DINR ) - 0.17 * Math.sin( ( 217.6 - 407332.20 * jCent ) / DINR );

		double pi = 0.9508 + 0.0518 * Math.cos( ( 134.9 + 477198.85 * jCent ) / DINR ) + 0.0095 * Math.cos( ( 259.2 - 413335.38 * jCent ) / DINR ) + 0.0078 * Math.cos( ( 235.7 + 890534.23 * jCent ) / DINR ) + 0.0028 * Math.cos( ( 269.9 + 954397.70 * jCent ) / DINR );
		...
		tc = new TopocentricCoords( gc.getx() , gc.gety() , gc.getz() , latitude , lstInDeg );
