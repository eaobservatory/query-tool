...
public class LocalSiderealTime
{
	...
	public double ut2lst()
	{
		...
		/*
		 * Equation of the equinoxes (requires TT although makes very little difference)
		 * Increment to be applied to Coordinated Universal Time UTC to give Terrestrial Time TT (formerly Ephemeris Time ET). 
		 * #Dtt(double)
		 */
		double terrestrialTime = modifiedJulianDate + ( pal.Dtt( modifiedJulianDate ) / 86400. ) ;
		/*
		 ...
		 * Normalize angle into range 0-2 . #Dranrm(double)
		 */
	...
	public void setTelescope( String telescope )
	{
		if( telescope != null && !telescope.trim().equals( "" ) )
		{
			...
			{
				throw new RuntimeException( "Telescope " + telescope + " unknown" , npe ) ;
			}
	...
	private double getMJD()
	{
		...
		{
			throw new RuntimeException( "Could not obtain fraction of day for " + hour + ":" + minute + ":" + second , pe ) ;
		}

		/*
		 * Calculate modified julian date of UT day
		 * Gregorian calendar to Modified Julian Date. 
		 * #Cldj(int,%20int,%20int)
		 */
		...
		{
			throw new RuntimeException( "Could not obtain modified julian date for " + month + "/" + day + "/" + year , pe ) ;
		}
