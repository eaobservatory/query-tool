...
public class AttributeEditor extends JDialog implements ActionListener,
     ...
     * @param haveScaledThisObs <code>true</code> if this has bee previouslty
     *            scaled.
     * @param oldFactor The last scaling factor used
     * @param rescale <code>true</code> if rescaling required.
     */
    public AttributeEditor(SpObs observation, java.awt.Frame parent,
            boolean modal, String configAttribute, boolean haveScaledThisObs,
            Double oldFactor, boolean rescale) {
        super(parent, "Attribute Scaling", modal);
        commonInitialisation(observation);
        initScaleComponents(configAttribute, haveScaledThisObs, oldFactor,
                rescale);
        doingScale = true;
    }
    ...
    private String concatPath(String path, SpItem newBit) {
        ...
        if (path.equals("")) {
            /*
             * Path is empty, no need for a separator. Skip over the observation
             * and the sequence if at the start of the path. This stops all the
             * tree attributes beginning with Observation/Sequence and keeps the
    ...
    private void createComponents(String attributes, String iterators,
        ...
        // Get the instrument attributes that are editable, along with their
        // current values.
        ...
        warnEmpty = new JLabel(
                "No attributes found which match the config spec.");
        ...
        // (without it, the scaleFactor is the component which grows with the
        // window, which is OK width wise but bogging height wise)
    ...
    private void initTableComponents() {
        ...
        if (tableIsEmpty) {
            // No attributes to edit. Just put a warning to this effect and an
            // OK button
            warnPanel.add(warn);
            ...
            // There are attributes to edit. Show the table, the OK button and
            // the Cancel button.
    ...
    private void initScaleComponents(String attribute,
        ...
        createComponents(instName + "_" + attribute, instName + "_ITERATORS",
                oldFactor);
        tableIsEmpty = (model.getRowCount() == 0);

        if (tableIsEmpty) {
            // No attributes to scale. Just put a warning to this effect and an
            // OK button
            warnPanel.add(warn);
            warnPanel.add(warnEmpty);
            getContentPane().add(warnPanel);
        ...
        if (doingScale) {
            try {
                ...
                message = "Invalid scale factor (" + scaleFactor.getText()
                        + ").";
                JOptionPane.showMessageDialog(this, message,
                        "Unable to comply", JOptionPane.ERROR_MESSAGE);
     ...
     * Recursively walk the tree finding (iterator, attribute, value) triplets
     * thus specified.
     *
     * @param SpObs observation
     * @author David Clarke
     ...
     *
     */
    private void getLocalIterAttValues(SpItem root, String subtype,
            String path, String indent) {
